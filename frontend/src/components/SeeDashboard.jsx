...
export default function SeeDashboard({ currentPlanId, onNextPlanCreated }) {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        const { data: todos } = await supabase.from('todos').select('*').eq('plan_id', currentPlanId);
        const { data: dos } = await supabase.from('dos').select('*, todos!inner(*)').eq('todos.plan_id', currentPlanId);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '16px' }}>
        ...
        <span style={{ fontSize: '0.75rem', backgroundColor: '#f8fafc', color: '#334155', padding: '4px 8px', borderRadius: '4px', fontWeight: '600', border: '1px solid #cbd5e1' }}>Target ID: {stats.plan_id.slice(0, 8)}</span>
      </div>
      ...
      <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '10px', border: '1px solid #e2e8f0', marginTop: '8px' }}>
        ...
        <form onSubmit={handleCreateNextPlan} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <input value={nextPlanTitle} onChange={e => setNextPlanTitle(e.target.value)} placeholder="다음 계획 주기 제목 (예: 정보처리기사 실기 2주차)" style={{ padding: '10px 14px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.9rem', backgroundColor: '#ffffff', outline: 'none' }} />
          <textarea value={adjustment} onChange={e => setAdjustment(e.target.value)} placeholder="회고 내용 및 다음 주기 반영 사항 작성..." rows="3" style={{ padding: '10px 14px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.9rem', backgroundColor: '#ffffff', outline: 'none', resize: 'vertical' }} />
