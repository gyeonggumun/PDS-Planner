...
export default function PlanToDoManager({ scope, onSelectPlan }) {
  ...
  const [planForm, setPlanForm] = useState({ title: '', startDate: todayStr, endDate: nextWeekStr });
  ...
  const loadData = async () => {
    try {
      ...
      const { data: tData } = await supabase.from('todos').select('*');
      setPlans(pData || []);
      setTodos(tData || []);
    } catch (e) {
  ...
  const handleCreatePlan = async (e) => {
    ...
    if (planForm.startDate > planForm.endDate) return alert('종료일은 시작일보다 빠를 수 없습니다.');
  ...
  const handleCompleteTodo = async (todoId) => {
    ...
    try {
      await supabase.from('todos').update({ status: 'completed' }).eq('id', todoId);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
        ...
        <form onSubmit={handleCreatePlan} style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
          <input placeholder="목표 제목 입력 (예: 정보처리기사 실기 대비)" value={planForm.title} onChange={e => setPlanForm({...planForm, title: e.target.value})} style={{ flex: 2, minWidth: '220px', padding: '10px 14px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.9rem', backgroundColor: '#ffffff', outline: 'none' }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', color: '#475569' }}>
            <span>시작:</span>
            <input type="date" value={planForm.startDate} onChange={e => setPlanForm({...planForm, startDate: e.target.value})} style={{ padding: '9px 10px', borderRadius: '6px', border: '1px solid #cbd5e1', backgroundColor: '#ffffff', fontSize: '0.85rem', outline: 'none' }} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', color: '#475569' }}>
            <span>종료:</span>
            <input type="date" value={planForm.endDate} onChange={e => setPlanForm({...planForm, endDate: e.target.value})} style={{ padding: '9px 10px', borderRadius: '6px', border: '1px solid #cbd5e1', backgroundColor: '#ffffff', fontSize: '0.85rem', outline: 'none' }} />
          </div>
          <button type="submit" style={{ padding: '10px 20px', backgroundColor: '#0f172a', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '0.9rem', fontWeight: '500', cursor: 'pointer' }}>계획 생성</button>
        </form>
      </div>
          ...
          return (
            <div key={plan.id} style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    <span style={{ fontSize: '0.75rem', backgroundColor: '#eff6ff', color: '#1d4ed8', padding: '2px 8px', borderRadius: '4px', fontWeight: '600', border: '1px solid #bfdbfe' }}>기간: {plan.period}</span>
                  </div>
                  <h4 style={{ margin: 0, fontSize: '1.05rem', fontWeight: '600', color: '#0f172a' }}>{plan.title}</h4>
                </div>
                <button onClick={() => onSelectPlan(plan.id)} style={{ backgroundColor: '#ffffff', color: '#0f172a', border: '1px solid #cbd5e1', padding: '6px 12px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: '500', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  회고(See) 분석 리포트
                </button>
              </div>
              ...
              <form onSubmit={(e) => handleCreateTodo(plan.id, e)} style={{ display: 'flex', gap: '8px' }}>
                <input placeholder="세부 과제(ToDo) 추가..." value={todoContents[plan.id] || ''} onChange={e => setTodoContents({...todoContents, [plan.id]: e.target.value})} style={{ flex: 1, padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.85rem', outline: 'none' }} />
                <button type="submit" style={{ padding: '8px 14px', backgroundColor: '#f1f5f9', color: '#334155', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '0.85rem', fontWeight: '500', cursor: 'pointer' }}>추가</button>
              </form>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  ...
                  return (
                    <li key={todo.id} style={{ backgroundColor: isDone ? '#f8fafc' : '#ffffff', border: '1px solid #f1f5f9', padding: '10px 12px', borderRadius: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', opacity: isDone ? 0.75 : 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ fontSize: '0.85rem', color: isDone ? '#16a34a' : '#94a3b8', fontWeight: 'bold' }}>{isDone ? '✓' : '•'}</span>
                        <span style={{ textDecoration: isDone ? 'line-through' : 'none', color: isDone ? '#64748b' : '#0f172a', fontSize: '0.9rem', fontWeight: isDone ? '400' : '500' }}>{todo.content}</span>
                      </div>
                      {!isDone && (
                        <button onClick={() => handleCompleteTodo(todo.id)} disabled={todo.isCompleting} style={{ backgroundColor: '#0f172a', color: '#ffffff', border: 'none', padding: '5px 12px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '500', cursor: 'pointer' }}>
                ...
                {planTodos.length === 0 && <li style={{ fontSize: '0.85rem', color: '#94a3b8', padding: '8px 0', textAlign: 'center' }}>등록된 세부 과제가 없습니다.</li>}
              </ul>
            </div>
          );
        })}
